...
export default function PCCChecklistFormList() {
      ...
      render: (_: any, record) => (
        <Space size="small">
          <Button
            type="text"
            icon={<EyeOutlined />}
            onClick={() => navigate(`/dashboard/inspection/forms/pcc-checklist/view/${record.id}`)}
            title="View"
          />
          {canEdit(record) && (
            <Button
              type="text"
              icon={<EditOutlined />}
              onClick={() => navigate(`/dashboard/inspection/forms/pcc-checklist/edit/${record.id}`)}
              title="Edit"
            />
          )}
          {canDelete(record) && (
  ...
  const headerActions = canCreate ? [
    <Button
      ...
      type="primary"
      icon={<PlusOutlined />}
      onClick={() => navigate('/dashboard/inspection/forms/pcc-checklist/create')}
    >
      New Form
    </Button>
  ] : [];
