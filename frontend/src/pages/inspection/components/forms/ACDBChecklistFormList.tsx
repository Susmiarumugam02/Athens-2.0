...
// Derive status from stored check fields ("status|remarks" format)
function deriveStatus(record: any): string {
  if (record.status) return record.status;
  const p1Keys = Array.from({ length: 16 }, (_, i) => `page1_check_${i}`);
  const p2Keys = Array.from({ length: 12 }, (_, i) => `page2_check_${i}`);
  const allKeys = [...p1Keys, ...p2Keys];
  const filled = allKeys.filter(k => record[k] && record[k] !== '|' && record[k] !== '');
  if (!filled.length) return 'draft';
  if (allKeys.some(k => (record[k] || '').startsWith('not_ok'))) return 'in_progress';
  if (filled.length < allKeys.length) return 'in_progress';
  return 'completed';
}
...
function deriveScore(record: any): number | null {
  ...
  const p2Keys = Array.from({ length: 12 }, (_, i) => `page2_check_${i}`);
  const allKeys = [...p1Keys, ...p2Keys];
  const scoreable = allKeys.filter(k => {
    const v = (record[k] || '').split('|')[0];
    return v === 'ok' || v === 'not_ok';
  });
  ...
  const columns = [
    {
      ...
      render: (_: any, record: any) =>
        `ACDB CHECKLIST - ${record.location || record.client || '—'}`,
    },
    {
      title: 'Status',
      key: 'status',
