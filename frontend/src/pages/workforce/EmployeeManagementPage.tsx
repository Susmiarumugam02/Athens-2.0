  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err?.response?.status === 403) {
        setError('Permission denied. Ensure you are a project admin with Workforce service enabled.')
  ...
  const fetchEmployees = () => {
    ...
    Promise.all([
      ...
      .catch(err => {
        const data = err?.response?.data
        if (err?.response?.status === 403) {
          setError('Workforce service is not enabled. Run: python manage.py enable_workforce_service')
        } else {
