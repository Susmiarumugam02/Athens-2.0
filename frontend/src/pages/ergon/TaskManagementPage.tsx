...
}> = ({ open, title, initialValues, projects, users, loading, hideProject, onClose, onSubmit }) => {
...
export default function TaskManagementPage() {
  ...
  const handleCreate = async (values: FormState) => {
    ...
    try {
      ...
      await ergonApi.createTask(payload)
  ...
  const handleEdit = async (values: FormState) => {
    ...
    try {
      ...
      if (!hideProject && values.project) payload.project = Number(values.project)
      await ergonApi.updateTask(editingTask.id, payload)
