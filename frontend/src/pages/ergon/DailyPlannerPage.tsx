...
export default function DailyPlannerPage() {
  ...
  const handleRolloverTasks = async () => {
    ...
    try {
      ...
      await apiClient.post('/api/ergon/planner/rollover/', { date: today })
      message.success('Incomplete tasks rolled over to today')
      // When real API is connected, refresh tasks here
    } catch (error: any) {
      message.error(error?.response?.data?.detail ?? 'Failed to rollover tasks')
    } finally {
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-start justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          />
          <Popconfirm
            title="Rollover incomplete tasks?"
            description="All not-started and in-progress tasks from yesterday will be copied to today."
            onConfirm={handleRolloverTasks}
            okText="Yes, Rollover"
            cancelText="Cancel"
          >
            <button
