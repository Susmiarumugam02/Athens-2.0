...
import { type Employee, type EmployeeFilters } from '../types/employeeTypes'
...
const EmployeeList: React.FC<EmployeeListProps> = ({
  ...
  const fetchEmployees = useCallback(async () => {
    ...
    try {
      ...
      const response = await api.get(`/api/athens-sustainability/employees/?${params.toString()}&session_key=${sessionKey}`, {
  ...
  const handleDeleteEmployee = async (employee: Employee) => {
    ...
    try {
      await api.delete(`/api/athens-sustainability/employees/${employee.id}/?session_key=${sessionKey}`, {
        headers: { Authorization: `Bearer ${sessionKey}` }
      })
  ...
  const handleTerminateEmployee = async (employee: Employee) => {
    ...
    try {
      await api.patch(`/api/athens-sustainability/employees/${employee.id}/?session_key=${sessionKey}`, {
        status: 'terminated',
        termination_reason: reason,
        termination_date: terminationDate
      })
      
      toast.success('Team member terminated successfully')
      fetchEmployees()
    } catch (error) {
      toast.error('Failed to terminate team member')
    }
  }
    ...
    <Card className="bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl border-gray-200/50 dark:border-gray-700/50">
      <CardHeader>
        <div className="flex items-center justify-between">
          ...
          <Button onClick={onAddEmployee} className="bg-gradient-to-r from-green-500 to-emerald-600">
      ...
      <CardContent>
        {loading ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-gray-500 dark:text-gray-400 mb-4">Get started by adding your first team member</p>
            <Button onClick={onAddEmployee} className="bg-gradient-to-r from-green-500 to-emerald-600">
              <Plus className="h-4 w-4 mr-2" />
              Add Team Member
            </Button>
              ...
              <div key={employee.id} className="bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-900 rounded-xl p-6 border border-gray-200/50 dark:border-gray-700/50 hover:shadow-md transition-all duration-200">
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-4">
                    <div className="h-12 w-12 rounded-full bg-gradient-to-r from-green-500 to-emerald-600 flex items-center justify-center text-white font-medium">
