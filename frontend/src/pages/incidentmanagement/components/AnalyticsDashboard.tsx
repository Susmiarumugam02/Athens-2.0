    ...
    <div>
      ...
      <Row gutter={16} style={{ marginBottom: 24 }}>
        <Col xs={24} sm={12} md={6}>
          <Card><Statistic title="Total Incidents" value={total_incidents} styles={{ content: { color: '#1890ff' } }} prefix={<WarningOutlined />} /></Card>
        </Col>
        <Col xs={24} sm={12} md={6}>
          <Card><Statistic title="Open Incidents" value={open_incidents} styles={{ content: { color: '#faad14' } }} prefix={<ClockCircleOutlined />} /></Card>
        </Col>
        <Col xs={24} sm={12} md={6}>
          <Card>
            <Statistic title="Closure Rate" value={closureRate} precision={1} suffix="%" styles={{ content: { color: closureRate >= 80 ? '#52c41a' : '#faad14' } }}
              prefix={closureRate >= 80 ? <RiseOutlined /> : <FallOutlined />} />
          </Card>
        </Col>
        <Col xs={24} sm={12} md={6}>
          <Card><Statistic title="Total Cost" value={analytics.total_cost} precision={0} styles={{ content: { color: '#ff4d4f' } }} prefix={<WalletOutlined />} /></Card>
        </Col>
      </Row>
      ...
      <Row gutter={16}>
        <Col span={24}>
          <Card title="Key Performance Indicators">
            <Row gutter={16}>
              <Col xs={24} sm={8}>
                <Statistic title="Avg Time to Close" value={analytics.average_time_to_close} suffix="days" styles={{ content: { color: '#1890ff' } }} />
              </Col>
              <Col xs={24} sm={8}>
                <Statistic title="Avg Cost per Incident" value={analytics.average_cost_per_incident} precision={0} prefix="$" styles={{ content: { color: '#faad14' } }} />
              </Col>
              <Col xs={24} sm={8}>
                <Statistic title="Overdue Incidents" value={overdue_incidents} styles={{ content: { color: overdue_incidents > 0 ? '#ff4d4f' : '#52c41a' } }} />
              </Col>
            </Row>
          </Card>
        </Col>
