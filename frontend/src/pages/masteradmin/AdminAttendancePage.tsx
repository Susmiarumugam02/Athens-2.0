...
import type { AdminAttendanceRecord, AttendanceDashboard, CorrectionPayload } from '../../services/adminAttendanceApi'
import { adminAttendanceApi } from '../../services/adminAttendanceApi'
...
function fmt(dt: string | null | undefined): string {
  if (!dt) return '—'
  try {
    // Handle time-only strings like "10:01:45" directly
    if (/^\d{2}:\d{2}/.test(dt)) {
      return dt.slice(0, 5)
    }
...
const AdminAttendancePageInner: React.FC = () => {
  ...
  const [filterAdminType, setFilterAdminType] = useState('project_admin')
  ...
  const fetchData = useCallback(async (filters: typeof filtersRef.current) => {
    ...
    try {
      const [dashRaw, listRaw] = await Promise.allSettled([
        adminAttendanceApi.getDashboard(filters.filterDate),
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50">
                  {['Admin Name', 'Type', 'Organization', 'Project', 'Date', 'Check In', 'Check Out', 'Hours', 'Status', 'GPS', 'Actions'].map(h => (
