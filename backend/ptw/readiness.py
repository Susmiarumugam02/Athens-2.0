...
def get_permit_readiness(permit):
    ...
    requires = {
        ...
        'signatures': True  # Always require signatures
    }
    ...
    details = {
        ...
        'signatures': _get_signature_details(permit)
    }
...
def _can_verify(permit):
    ...
    if permit.status not in ['submitted', 'draft']:
        return False
    
    # Check if verifier signature is present
    if permit.verifier:
        try:
            validate_required_signatures_for_action(permit, 'verify', permit.verifier)
            return True
        except Exception:
            return False
    
    return True  # Can verify if no verifier assigned yet
...
def _can_approve(permit):
    ...
    try:
        ...
        # Check if approver signature is present
        approver = permit.approver or permit.approved_by
        if approver:
            validate_required_signatures_for_action(permit, 'approve', approver)
...
def _get_missing_for_approve(permit):
    ...
    if approver:
        try:
            ...
            missing.append('approver_signature_missing')
...
def _get_signature_details(permit):
    """Get digital signature details"""
    signatures = DigitalSignature.objects.filter(permit=permit)
    signature_map = {sig.signature_type: sig for sig in signatures}
    
    requestor_sig = signature_map.get('requestor')
    verifier_sig = signature_map.get('verifier')
    approver_sig = signature_map.get('approver')
    
    return {
        'requestor': {
            'required': True,
            'present': bool(requestor_sig and requestor_sig.signature_data),
            'signed_by': requestor_sig.signatory.get_full_name() if requestor_sig else None,
            'signed_at': requestor_sig.signed_at.isoformat() if requestor_sig else None,
            'required_user': permit.created_by.get_full_name() if permit.created_by else None
        },
        'verifier': {
            'required': bool(permit.verifier),
            'present': bool(verifier_sig and verifier_sig.signature_data),
            'signed_by': verifier_sig.signatory.get_full_name() if verifier_sig else None,
            'signed_at': verifier_sig.signed_at.isoformat() if verifier_sig else None,
            'required_user': permit.verifier.get_full_name() if permit.verifier else None
        },
        'approver': {
            'required': bool(permit.approver or permit.approved_by),
            'present': bool(approver_sig and approver_sig.signature_data),
            'signed_by': approver_sig.signatory.get_full_name() if approver_sig else None,
            'signed_at': approver_sig.signed_at.isoformat() if approver_sig else None,
            'required_user': (permit.approver or permit.approved_by).get_full_name() if (permit.approver or permit.approved_by) else None
        }
    }
