...
def create_toolbox_talk(request):
    ...
    if serializer.is_valid():
        training = serializer.save(created_by=request.user, project=project)
        logger.info(f"Training created: ID={training.id}, Title={training.title}, Project={project.id if project else None}, User={request.user.id}")
        return Response(serializer.data, status=status.HTTP_201_CREATED)
    
    logger.warning(f"Training creation failed: Errors={serializer.errors}")
...
class ToolboxTalkViewSet(TenantScopedViewSet):
    ...
    def get_queryset(self):
        ...
        if user.is_superuser:
            logger.debug(f"Superuser {user.id} - returning all trainings")
            return ToolboxTalk.objects.all()

        queryset = super().get_queryset()
        user_project = getattr(user, 'project', None)
        logger.debug(f"Listing trainings for user {user.id}, project {user_project.id if user_project else None}")
