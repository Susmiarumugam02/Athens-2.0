...
def serialize_auth_user(user):
    ...
    is_temporary_password = bool(getattr(user, 'is_temporary_password', False))
    return {
        'id': user.id,
        'email': user.email,
        ...
        'attendance_verified': getattr(user, 'attendance_verified', getattr(user, 'attendance_status', '') == 'verified'),
        'modules_unlocked': getattr(user, 'modules_unlocked', getattr(user, 'module_access_enabled', False)),
        'access_status': getattr(user, 'access_status', 'active' if getattr(user, 'module_access_enabled', False) else 'restricted'),
        'onboarding_completed': getattr(user, 'onboarding_completed', getattr(user, 'onboarding_status', '') == 'completed'),
        ...
        'must_reset_password': must_change_password,
        'is_password_reset_required': bool(getattr(user, 'is_password_reset_required', False)),
        'is_autogenerated_password': bool(getattr(user, 'is_autogenerated_password', False)),
...
def unified_login(request):
    ...
    has_role_user = getattr(user, 'role_type', 'user') == 'user'
    if (
    ...
    # Approval gating — only block users who have submitted profile but not yet approved
    # Users with pending_profile status get tokens so they can submit their profile
    approval_status = getattr(user, 'approval_status', 'pending')
    user_status = getattr(user, 'status', 'active')
    role_type = getattr(user, 'role_type', 'admin')

    # Regular users (role_type=user) must go through onboarding flow
    if role_type == 'user' and user.user_type == UserType.COMPANYUSER:
        ...
        if approval_status == 'rejected':
            ...
            return Response(
                {'error': 'Your account has been rejected. Contact your administrator.', 'approval_status': 'rejected'},
                status=status.HTTP_403_FORBIDDEN,
            )
        # pending_approval users: tokens issued but redirected to waiting screen
        # pending_profile users: tokens issued, redirected to profile form
        # approved_pending_induction: tokens issued, redirected to induction page
        # active: full access
    elif user.user_type not in [UserType.SUPERADMIN, UserType.MASTERADMIN]:
        # Non-regular-user company users: enforce approval
        if approval_status not in ('approved', 'pending') and user_status not in ('active', 'pending_profile', 'pending_approval', 'approved_pending_induction'):
            log_security_event(
                request, user, SecurityLog.EventType.LOGIN_FAILED,
                SecurityLog.Severity.WARNING,
                {'reason': 'approval_not_granted', 'approval_status': approval_status},
            )
            return Response(
                {'error': 'Your account is pending admin approval.', 'approval_status': approval_status},
                status=status.HTTP_403_FORBIDDEN,
            )

    # Generate tokens (only approved users)
    ...
    # Determine next_route based on new status field
    role_type = getattr(user, 'role_type', 'user')
    user_status = getattr(user, 'status', 'active')
    ...
    if user.user_type == UserType.SUPERADMIN:
        ...
        if role_type == 'user':
            # Regular user - enforce strict onboarding flow
            profile_completed = getattr(user, 'profile_completed', False)
            training_status = getattr(user, 'training_status', 'not_started')
            induction_done = (
                getattr(user, 'induction_completed', False) is True or
                getattr(user, 'induction_attended', False) is True or
                getattr(user, 'induction_status', '') == 'completed'
            )
            attendance_done = (
                getattr(user, 'attendance_verified', False) is True or
                getattr(user, 'attendance_status', '') == 'verified'
            )
            modules_enabled = (
                getattr(user, 'modules_unlocked', False) is True or
                getattr(user, 'module_access_enabled', False) is True
            )
            active_access = (
                getattr(user, 'access_status', 'restricted') == 'active' or
                getattr(user, 'status', '') == 'active' or
                getattr(user, 'access_level', '') == 'full_access'
            )
            onboarding_done = (
                getattr(user, 'onboarding_completed', False) is True or
                getattr(user, 'onboarding_status', '') == 'completed'
            )
            has_full_access = induction_done and attendance_done and modules_enabled and (active_access or onboarding_done)
            if not profile_completed or user_status == 'pending_profile':
                next_route = '/user/complete-profile'
            elif approval_status == 'waiting_admin_approval' or user_status == 'pending_approval':
                next_route = '/user/approval-pending'
            elif has_full_access:
                next_route = '/app/dashboard'
            elif approval_status == 'approved' and training_status != 'completed':
                # CRITICAL: User can access training only until induction attendance is verified
                next_route = '/user/induction-pending'
            else:
                # Fallback for legacy users without status field
                is_first_login = getattr(user, 'is_first_login', False)
                ...
                if is_first_login or not profile_completed:
                    next_route = '/user/complete-profile'
                elif approval_status in ('pending', 'waiting_admin_approval'):
                    next_route = '/user/approval-pending'
...
@api_view(['POST'])
@permission_classes([IsAuthenticated])
def company_user_change_password(request):
    """Change password for any authenticated platform user."""
    user = request.user

    current_password = request.data.get('current_password', '').strip()
    new_password = request.data.get('new_password', '').strip()
    confirm_password = request.data.get('confirm_password', '').strip()
    force_logout_all = bool(request.data.get('force_logout_all', False))
    ...
    SecurityLog.objects.create(
        ...
        user=user,
        metadata={
            'event': 'company_user_password_change',
            'user_id': user.id,
...
def departments_list(request):
    """Return departments for the current user's tenant."""
    try:
        from ergon.models import Department
        from authentication.tenant_utils import get_current_tenant
        tenant, _ = get_current_tenant(request.user)
        if tenant:
            depts = Department.objects.filter(athens_tenant_id=tenant.id).values('id', 'name')
        else:
